...
class Basemiddelware:
    ...
    def __call__(self, request):
        ...
        print('start',self.middelware_name)
        even_squares = [i * i for i in range(100) if i % 2 == 0]
        response = self.get_responce(request)
        print('end',even_squares)

class First(Basemiddelware):
    middelware_name = 'First middelware'

class Second(Basemiddelware):
    middelware_name = 'Second middelware'

class Third(Basemiddelware):
    middelware_name = 'Third middelware'